...
let CollectionViewHeaderViewID = "CollectionViewHeaderView"

class CollectionViewLinkageVC: RootViewController {

    fileprivate lazy var tableView : UITableView = {
        let tableView = UITableView()
        tableView.delegate = self
        tableView.dataSource = self
        tableView.frame = CGRect(x: 0, y: 0, width: 80, height: kScreenH)
        tableView.rowHeight = 55
        tableView.showsVerticalScrollIndicator = false
        tableView.separatorColor = UIColor.clear
        tableView.register(LeftTableViewCell.self, forCellReuseIdentifier: LeftTableViewCellID)
        return tableView
    }()
    ...
    fileprivate lazy var collectionView: UICollectionView = {
        ...
        collectionView.register(CollectionViewHeaderView.self, forSupplementaryViewOfKind: UICollectionElementKindSectionHeader, withReuseIdentifier: CollectionViewHeaderViewID)
...
//MARK: - 获取数据
extension CollectionViewLinkageVC {
    func configureData() {
...
extension CollectionViewLinkageVC: UITableViewDelegate, UITableViewDataSource {
...
extension CollectionViewLinkageVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let model = collectionDatas[indexPath.section][indexPath.row]
        printLog("collectionView ====> \(String(describing: model.name))")
    }
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        var reuseIdentifier : String?
        if kind == UICollectionElementKindSectionHeader {
            reuseIdentifier = CollectionViewHeaderViewID
        }

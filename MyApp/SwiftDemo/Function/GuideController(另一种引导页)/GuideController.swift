...
class GuideController: UIViewController {
    ...
    public typealias IndexChangedBlock = ((_ index: Int, _ item: GuideItem) -> Void)
    ...
    private var currentItem: GuideItem {
        get {
    ...
    public convenience init(item: GuideItem, key: String?) {
        self.init(items: [item], key: key)
    }

    public init(items: [GuideItem], key: String?) {
    ...
    public func show(from vc: UIViewController, completion: CompletionBlock?) {
        ...
        if let key = guideKey {
            if GuideDataManager.shouldShowGuide(with: key) {
    ...
    private func configViews() {
        ...
        if let image = arrowImage {
            arrowImageView.image = image.xl_image(with: arrowColor)
        } else {
            arrowImageView.image = UIImage(named: "guide_arrow", in: Bundle(for: GuideController.self), compatibleWith: nil)?.xl_image(with: arrowColor)
        }
    ...
    private func configViewFrames() {
        ...
        let size = currentItem.text.xl_getSize(of: font, maxWidth: maxWidth)
